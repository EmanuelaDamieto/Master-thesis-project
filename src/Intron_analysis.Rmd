...
spruce <- read.table(gzfile(here("reference/gff3/Picab02_codingAll.gff3.gz")), sep = "\t", quote="")
...
euler_expr_genes <- euler(c(not_expr_C2D = 3709, not_expr_CNT = 1799, "not_expr_C2D&not_expr_CNT" = 6566))
...
expr_genes <- expr_common$Gene
write.table(expr_genes, here("data/analysis/expressed_genes.txt"), row.names=FALSE, col.names=FALSE, quote=FALSE)
...
```{r comparison of genes expressed in different conditions}
vst_mean <- vst %>% transmute(Gene=Gene, "80%" = rowMeans(vst[,2:5]), "60%" = rowMeans(vst[,6:9]), "40%" = rowMeans(vst[,10:13]),"30%" = rowMeans(vst[,14:17]), "30%7d" = rowMeans(vst[,18:20]), "Collapse" = rowMeans(vst[,21:23]), C2d=rowMeans(vst[,24:26]), "Rehydrate" = rowMeans(vst[,27:29]))

#Check how many genes are expressed in C2d, Rehydrate, 307d


#there are 10 275 genes that are not expressed in C2d
#8265 not expressed in control
#there are 8096 genes that are not expressed in rehydrate
no_expr_rehydr <- filter(vst_mean, Rehydrate==0) 
#there are 8829 genes that are not expressed in the 307d
no_expr_307d <- filter(vst_mean, `30%7d`==0)
#6173 of these are not expressed in Rehydrated and control
no_expr_rehydr_cnt <- filter(vst_mean, Rehydrate==0 & `80%`==0)
#6583 of these are not expressed in 307d and control
no_expr_307d_cnt <- filter(vst_mean, `30%7d`==0 & `80%`==0)
#6566 not expressed in C2d and control
...
#normalized since we used the dds with the option countsFromAbundance="lengthScaledTPM"
...
1. Consider the first intron length of the most representative mRNA of each gene 
...
mRNA_repre <- read.table(here("reference/gff3/Picab02_codingRepresentative.gff3.gz"), sep = "\t", quote="")
